...
class UltraVehicleCard extends LitElement {
		...
		return html`
			<ha-card>
				<div class="vehicle-card-content">
					${this._renderHeader()} ${this._renderCarState()} ${this._renderVehicleImage()} ${this._renderIconGrid()}
				...
				${this.config.show_battery && batteryLevel !== null
					? html`
							<div class="item_bar">
								...
								${chargeLimit !== null
									? html` <div class="charge-limit-indicator" style="left: ${chargeLimit}%;"></div> `
									: ''}
							</div>
							<div class="level-text">
								<span>${batteryLevel}% ${isCharging ? 'Charging' : 'Charge'}</span>
								${this.config.show_battery_range && batteryRange !== null
									? html` <span class="range">${batteryRange} ${this.config.unit_type}</span> `
									: ''}
							</div>
					  `
					: this.config.show_battery_range && batteryRange !== null
					...
					? html`
							...
							<div class="level-text">
								...
								${this.config.show_fuel_range && fuelRange !== null
									? html` <span class="range">${fuelRange} ${this.config.unit_type}</span> `
									: ''}
							</div>
					  `
					: this.config.show_fuel_range && fuelRange !== null
			...
			${this.config.show_battery && level !== null
				? html`
						...
						<div class="level-text">
							...
							${this.config.show_battery_range && range !== null
								? html` <span class="range">${range} ${this.config.unit_type}</span> `
								: ''}
						</div>
				  `
				: this.config.show_battery_range && range !== null
			...
			${this.config.show_fuel && level !== null
				? html`
						...
						<div class="level-text">
							...
							${this.config.show_fuel_range && range !== null
								? html` <span class="range">${range} ${this.config.unit_type}</span> `
								: ''}
						</div>
				  `
	...
	_renderCarState() {
		...
		return html` <div class="car-state">${state}</div> `;
	}
	...
	_renderIconGrid() {
		...
		return html` <div class="icon-grid">${icon_grid_entities.map((entityId) => this._renderIconItem(entityId))}</div> `;
	}
