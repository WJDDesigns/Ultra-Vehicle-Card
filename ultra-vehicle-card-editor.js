...
export class UltraVehicleCardEditor extends LitElement {
	...
	render() {
		...
		return html`
			<div class="form">
				${this._renderBasicConfig()} ${this._renderEntityPickers()} ${this._renderIconGridConfig()}
			</div>
		`;
	}
			...
			<div class="icon-grid-container">
				...
				<div class="input-group">
					...
					<div class="entity-description">
						Search and select entities to add to the icon grid. Use the drag handle (≡) to reorder entities. Click on
						the icon to change it, and use the (×) to remove an entity from the grid.
					</div>
					<div class="entity-picker-container">
						<input
							id="icon_grid_search"
		...
		return html`
			<div class="entity-picker-results">
				${filteredEntities.map(
					(eid) => html`
						<div class="entity-picker-result" @click="${() => this._addIconGridEntity(eid)}">${eid}</div>
					`,
				)}
			</div>
		`;
		...
		return html`
			<div class="icon-picker-popup" @click="${(e) => e.stopPropagation()}">
				<ha-icon-picker
					...
					@value-changed=${this._handleIconChange}></ha-icon-picker>
			</div>
		`;
	}
	...
	async _handleImageUpload(ev) {
		...
		try {
			...
			if (this.config) {
				...
				console.log('background_images added', this.config.image_url);
				this.configChanged(this.config);
				this.requestUpdate();
			}
		} catch (error) {
